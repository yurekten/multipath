...
class FlowMultipathManager(object):
    ...
    def __init__(self, multipath_app, graph, dp_list, src, first_port, dst, last_port, ip_src, ip_dst,  max_paths=10, max_installed_path_count = 2, min_timeout_time=10, highest_priority=60000, *args, **kwargs):
        ...
        self.path_choise_flow_id = defaultdict()
        self.initial_tasks = []
    ...
    def initiate_paths(self):
        ...
        if self.state == FlowMultipathManager.NOT_ACTIVE:
            ...
            self.paths_with_ports = None
            self.initial_tasks =   []
            self.optimal_paths =  self.get_optimal_paths()
            ...
            path_indices = range(0, len(self.paths_with_ports))
        ...
        if first_initialization == True or self.state == FlowMultipathManager.ACTIVE:
            ...
            if len(self.installed_path_indices) < self.max_installed_path_count:
                for i in range(len(self.installed_path_indices), self.max_installed_path_count):
                    ...
                    current_path = self.path_choices[self.last_installed_path_index]
                    flows = {}
                    self.path_choise_flow_id[self.last_installed_path_index] = flows
                    timeout_time = timeout_time + self.min_timeout_time

                    priority = self.highest_priority - self.last_installed_path_index
                    #task = asyncio.create_task(self.create_flow_rules(self.paths_with_ports[current_path], flows, priority, hard_timeout=timeout_time))
                    #self.initial_tasks.append(task)
                    self.create_flow_rules(self.paths_with_ports[current_path], flows, priority, hard_timeout=timeout_time)
    ...
    def create_flow_rules(self, current_path, flow_id_list, priority, hard_timeout=10):
        for node in current_path:
            ...
            flow_id_list[flow_id] = (dp, match_ip, match_arp, actions)
    ...
    def flow_removed(self, msg):
        #print (msg)
        removed_path_index = None
        for choise in self.path_choise_flow_id:
            if msg.cookie in self.path_choise_flow_id[choise]:
                removed_path_index = choise
                del self.path_choise_flow_id[choise][msg.cookie]
                break
